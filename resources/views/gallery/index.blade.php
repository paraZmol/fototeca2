...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-section">
        <h4 class="sidebar-section-label">
            <svg class="sidebar-section-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> Distribución Geográfica
        </h4>
                ...
                <div x-data="{ subOpen: {{ request('location') && in_array(request('location'), array_merge([$district->id], $district->children->pluck('id')->all())) ? 'true' : 'false' }} }" class="accordion-group sub">
                    ...
                    @if($district->children->count() > 0)
                        <button @click="subOpen = !subOpen" class="accordion-btn accordion-btn--sub" :class="{'accordion-btn--open': subOpen}">
                            ...
                            <svg class="accordion-chevron" :class="{'rotated': subOpen}" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>
                        </button>
    ...
    {{-- ─── SECCIÓN 2: FOTÓGRAFOS CONSAGRADOS ─── --}}
    <div class="sidebar-section" style="margin-top: 1.5rem;">
        <h4 class="sidebar-section-label">
            <svg class="sidebar-section-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><circle cx="12" cy="14" r="3"/></svg> Fotógrafos Consagrados
        </h4>
        <a href="{{ route('photographers.index') }}" class="accordion-btn" style="text-decoration:none;margin-bottom:0.25rem;">
            <svg class="accordion-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            <span class="accordion-btn-text">Ver todos los fotógrafos</span>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
        </a>
        {{-- categorias de la seccion fotografos --}}
        @php $fotoCat = $rootCategories->firstWhere('slug', 'fotografos-consagrados'); @endphp
        @if($fotoCat)
        <div x-data="{ open: false }" class="accordion-group">
            <button @click="open = !open" class="accordion-btn" :class="{'accordion-btn--open': open}">
                ...
                <span class="accordion-btn-text">Colecciones Temáticas</span>
                <svg class="accordion-chevron" :class="{'rotated': open}" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            <div x-show="open" x-cloak class="accordion-panel">
                @foreach($fotoCat->children as $child)
                <a href="{{ route('galeria', array_merge(request()->except(['category','subcategory','page']), ['category' => $child->id])) }}"
                   class="sidebar-leaf {{ request('category') == $child->id ? 'sidebar-leaf--active' : '' }}">
                    <span class="leaf-dot"></span>{{ $child->name }}
                </a>
                @endforeach
            </div>
        </div>
    ...
    <div class="sidebar-section" style="margin-top: 1.5rem;">
        <h4 class="sidebar-section-label">
            <svg class="sidebar-section-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> Especiales
        </h4>
        @if($specialCollections)
            @foreach($specialCollections->children as $special)
            <a href="{{ route('specials.show', $special->slug) }}"
               class="accordion-btn" style="text-decoration:none;">
                <span style="font-size:14px;margin-right:4px;">{{ $special->icon }}</span>
                <span class="accordion-btn-text">{{ $special->name }}</span>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
            </a>
            @endforeach
        @endif
        <a href="{{ route('specials.index') }}" class="sidebar-leaf" style="margin-top:0.5rem;font-style:italic;">
